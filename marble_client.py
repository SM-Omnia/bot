...
from segmenting import plot_segments
from slic_kmeans import get_quadrant_sums, segment_image_slic_kmeans
...
try:
    ...
    logging.error(
        "Failed to import generated gRPC modules. "
...
class MarbleClient:
    ...
    def get_state(self) -> service_pb2.StateResponse:
        ...
        try:
            ...
            #
            # if response.screen is None:
            #     return response

            image_array = np.array(
                Image.frombytes("RGBA", (1280, 720), response.screen)
            )
            image = cv2.cvtColor(
                image_array,
                cv2.COLOR_BGR2RGB,
            ...
            segment_mask = segment_image_slic_kmeans(
                cv2.cvtColor(image, cv2.COLOR_BGR2RGB), n_segments=8
            )

            # Calculate the sums for each quadrant
            quadrant_sums = get_quadrant_sums(segment_mask)
            print("Pixel sums by quadrant:")
            for quadrant, sum_val in quadrant_sums.items():
                print(f"{quadrant}: {sum_val}")

            fig = plot_segments(image, segment_mask, quadrant_sums)

            # plt.show()

            output_path = f"DUMP/segments-{time.time()}.png"
    ...
    def send_input(
        self, input_request: service_pb2.InputRequest
    ) -> service_pb2.EmptyResponse:
        """
    ...
    def decision(self, state: service_pb2.StateResponse) -> service_pb2.InputRequest:
        ...
        return service_pb2.InputRequest(
            forward=forward, back=back, left=left, right=right, reset=reset
        )
            ...
            if current_state.finished:
                for index, result in enumerate(current_state.results):
                    if result.name == self.name:
                        ...
                        print(
                            f"Result {index}: {result.name}, Finish Time: {result.finish_time}, "
                ...
                results_list = [
                    {
                        ...
                        "last_touched_road_id": get_optional_uint64(
                            r.last_touched_road_id
                        ),
                        "last_touched_road_time": get_optional_float(
                            r.last_touched_road_time
                        ),
                    }
            ...
            data.append(
                {
                    ...
                    "screen": state[
                        "screen"
                    ],  # Keep as bytes, or process further if needed
